...
class AccountMoveLineExt(models.Model):
    ...
    def _compute_discount_amount(self):
        for rec in self:
            rec.discount_amount = rec.gross_amount * (rec.discount / 100.0) if rec.discount else 0.0
...
class AccountMoveExt(models.Model):
    ...
    @api.depends("invoice_line_ids.gross_amount")
    def _compute_total_gross_amount(self):
        for rec in self:
            rec.total_gross_amount = sum(line.gross_amount for line in rec.invoice_line_ids)

    @api.depends("invoice_line_ids.discount_amount")
    def _compute_total_discount_amount(self):
        for rec in self:
            rec.total_discount_amount = sum(line.discount_amount for line in rec.invoice_line_ids)
